...
levels(metadata$dilution)
metadata$dilution.ord = factor(metadata$dilution, c("0.1", "0.2", "0.3", "0.4"))

levels(metadata$frequency)
metadata$frequency.ord = factor(metadata$frequency, c("1", "4", "16", "600"))

levels(metadata$dilution.ord)
levels(metadata$frequency.ord)
...
physeq <- qza_to_phyloseq(
  features="core-metrics-results/table.qza",
...
my_column <- "dilution"  #this is the metadata column that we will use in the taxa barplot
...
for(ml in my_level){
  ...
  # Change this depending on whether we plot by dilution or by frequency
  physeq_meta_filtered$dilution = factor(physeq_meta_filtered$dilution, c("0.1", "0.2", "0.3", "0.4"))
  # physeq_meta_filtered$frequency = factor(physeq_meta_filtered$frequency, c("1", "4", "16", "600"))
  ...
  ggsave(paste0("output/", ml, "BarPlot_", my_column, ".svg"), height = 5, width = 4, device="pdf")
