...
#setwd(dir = "~C:\Users\Cole Swackhamer\Documents\ANSC516 R directory/")
#setwd("~/OneDrive/OneDrive Docs/Purdue/2023 Bioinformatics class/Project analysis/R")
# setwd("~/ANSC516 R directory/Cooccurence")
setwd("~/OneDrive/OneDrive Docs/Purdue/2023 Bioinformatics class/Project analysis/R")
...
str(metadata)
#colnames(metadata)[2] = "dilution"

dataset <- merge(metadata, dataset, by.x = "SampleID", by.y = 0)
treatments<-as.vector(unique(dataset$dilution))
datasetn<-dataset
datasetn[datasetn==0]<-NA

#i = 1

summary(metadata$dilution)

my_column <- "dilution" #change this to the number of factor levels of our factor
n1 <- 4
n2 <- 4
n3 <- 4
n4 <- 4

num_metadata_columns <- 5 # change to the number of cols in the metadata
...
strong_results<-subset(final_results, rho >= 0.75) # can change this
...
write.csv(subset(strong_results_taxa, trt=="dilution1"), "output/strong-results-taxa-dilution1.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="dilution2"), "output/strong-results-taxa-dilution2.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="dilution3"), "output/strong-results-taxa-dilution3.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="dilution4"), "output/strong-results-taxa-dilution4.csv", row.names = F)
